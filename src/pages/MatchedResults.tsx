...
const MatchResults: React.FC = () => {
  ...
  const { results, updateBlobs, session_id } = useMatch();
        ...
        if (res.status === 200 && res.data) {
          const merged = results.map((result) => {
            ...
            const blobFound = res.data.files.find((blob: Blob) => {
              return blob.url.includes(`${jd}/${fileNameOnly}`);
            });
  ...
  return (
    <div className='min-h-screen space-y-8 w-[95%] mx-auto'>
      ...
      {/* Main Panel */}
      <Panel size='md' title='Ranked Candidate List'>
